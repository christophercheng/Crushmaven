...
from fbdater.crush.models.user_models import FacebookUser
from django.db.models import F
from email import utils
import time
from django.db import transaction
import fbdater.crush.models.lineup_models
import fbdater.utils_email
...
class CrushRelationship(BasicRelationship):
    ...
    def handle_results_paid(self):
        ...
        related_setup_lineup_members = fbdater.crush.models.lineup_models.SetupLineupMember.objects.filter(relationship__target_person=self.source_person,username=self.target_person.username)
    ...
    def notify_target_person(self):
        ...
        if (target_person.bNotify_new_admirer== True):
            fbdater.utils_email.send_mail_new_admirer(self.friendship_type,full_name,short_name,first_name,target_person_email)
    ...
    def notify_source_person(self):
        ...
        if (target_status > 3 and source_person.bNotify_crush_responded==True): # user responded
            if self.is_results_paid == True: # target person changed their mind
                if target_status ==4:
                    fbdater.utils_email.send_mail_changed_attraction_response(True,full_name, short_name, first_name, pronoun_subject, pronoun_possessive, source_person_email)
                else:
                    fbdater.utils_email.send_mail_changed_attraction_response(False,full_name, short_name, first_name, pronoun_subject, pronoun_possessive, source_person_email)
                return
            else:
                ...
                fbdater.utils_email.send_mail_new_attraction_response(full_name, short_name, first_name, pronoun_subject, pronoun_possessive, source_person_email,send_time)
    ...
    def notify_source_person_bad_invite_email(self,bad_email_address):      
        ...
        short_name = target_person.first_name + " " + target_person.last_name[0]
        first_name = target_person.first_name
        fbdater.utils_email.send_mail_delivery_problem(full_name, short_name, first_name, bad_email_address, self.source_person.email)
...
class SetupRelationship(BasicRelationship):
    ...
    def notify_source_person(self,setup_lineup_member=None):
        ...
        if setup_lineup_member != None:      
            ...
            fbdater.utils_email.send_mail_setup_recommendee_response(full_name, short_name, first_name, pronoun_subject, pronoun_possessive, target_person.get_name(),setup_lineup_member.lineup_member_attraction, source_person_email)
        else:
            target_person=self.target_person
            full_name=target_person.get_name()
            short_name=target_person.first_name + ' ' + target_person.last_name[0]
            ...
            pronoun_subject = target_person.get_gender_pronoun_subject()
            pronoun_possessive = target_person.get_gender_pronoun_possessive()
            fbdater.utils_email.send_mail_setup_target_response(full_name, short_name, first_name, pronoun_subject, pronoun_possessive, source_person_email)
