...
import fbdater.crush.models.relationship_models
from fbdater.utils import graph_api_fetch
from fbdater.crush.models.miscellaneous_models import InviteEmail
import thread
...
class FacebookUserManager(UserManager):
    ...
    def handle_activated_user(self,user,fb_profile):
        ...
        admirer_relationships = fbdater.crush.models.relationship_models.CrushRelationship.objects.all_admirers(user)
...
class FacebookUser(AbstractUser):
    ...
    def html_for_inactive_friend_section(self,ajax_reprocess_friends_with_admirers=False): 
        ...
        for inactive_crush_friend in self.friends_with_admirers.all():
            ...
            all_admirers = fbdater.crush.models.relationship_models.CrushRelationship.objects.all_admirers(inactive_crush_friend)
    ...
    def get_progressing_admirers(self):
        return fbdater.crush.models.relationship_models.CrushRelationship.objects.progressing_admirers(self)
    # called by lineup.html to determine what to do after jquery lineup slider closes
...
class NamesLookup(object):
    ...
    def get_query(self,q,request):
        ...
        return fbdater.crush.models.relationship_models.CrushRelationship.objects.completed_crushes(request.user).filter(Q(target_status=4),Q(target_person__first_name__contains=q) | Q(target_person__last_name__icontains=q) )
