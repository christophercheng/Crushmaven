...
def initialize_fof_crush(request):
        ...
        access_token=post_data['access_token'] # relationship.source_person.access_token
        exclude_id_string=post_data['exclude_id_string'] #g_init_dict[crush_id]['exclude_id_string']
        rel_id=post_data['rel_id']
        admirer_id=post_data['admirer_id']
        crush_id=post_data['crush_id']
        admirer_gender=post_data['admirer_gender']
        minimum_lineup_members=int(post_data['minimum_lineup_members'])
        ideal_lineup_members=int(post_data['ideal_lineup_members'])
        
        relationship = {'rel_id' : rel_id,
                        'crush_id' : crush_id,
                        'admirer_id' : admirer_id,
                        'admirer_gender' : admirer_gender,
                        'minimum_lineup_members' : minimum_lineup_members,
                        'ideal_lineup_members' : ideal_lineup_members,
                        'access_token' : access_token,
                        'exclude_id_string' : exclude_id_string}
        ...
        if len(mutual_app_friend_array)>0:
            acceptable_id_array = try_api_mf_initialization(relationship,mutual_app_friend_array)
            if len(acceptable_id_array)>minimum_lineup_members:
                return create_lineup(relationship,acceptable_id_array)
        ...
        if 'body' in fb_result[4] and 'data' in fb_result[4][u'body']:
            ...
            if len(crush_app_friend_array) >= minimum_lineup_members:
                ...
                acceptable_id_array = try_api_cf_initialization(relationship,crush_app_friend_array)
                if len(acceptable_id_array)>minimum_lineup_members:
                    return create_lineup(relationship,acceptable_id_array)   
        ...
        if 'body' in fb_result[1] and 'data' in fb_result[1][u'body']:
            ...
            if 'body' in fb_result[3] and 'data' in fb_result[3][u'body']:
                ...
                try_nonapi_mf_initialization(relationship)
...
def try_api_mf_initialization(self,relationship,mutual_app_friend_array):
    ...
    minimum_lineup_members=relationship['minimum_lineup_members']
    ideal_lineup_members=relationship['ideal_lineup_members']
    admirer_gender = relationship['admirer_gender']
...
def try_api_cf_initialization(self,relationship,crush_app_friend_array):
    ...
    admirer_gender = relationship['admirer_gender']
    access_token = relationship['access_token']
    exclude_id_string = relationship['exclude_id_string']  
...
def try_nonapi_mf_initialization(self,relationship):
    ...
    if len(g_init_dict[crush_id][rel_id + '_mutual_friend_array']) > 0:
        ...
        process_mutual_friend(relationship,0)
    
    else:
        try_nonapi_cf_initialization(relationship)
...
def process_mutual_friend(self,relationship,mf_index):
    ...
    if num_friends < relationship['minimum_lineup_members'] or num_friends == None:
        finished_process_mutual_friend(relationship,mf_index)
        return
    else:
        ...
        process_batch_blocks(relationship,mf_index,q_block_array,0)
...
def process_batch_blocks(self,relationship,mf_index,q_block_array,q_start_index):
    ...
    for x in range(q_start_index,last_fetch_index):
        thread.start_new_thread(fetch_block,(relationship,mf_index,q_block_array,q_start_index,x)) #initialize lineup asynchronously            
...
def fetch_block(self,relationship,mf_index,q_block_array,q_start_index,q_index):
    ...
    try:
        ...
        fetch_block_finished(relationship,mf_index,q_block_array,q_start_index, [])
        return
    ...
    fetch_block_finished(relationship,mf_index,q_block_array,q_start_index,extracted_id_list)
...
def fetch_block_finished(self,relationship,mf_index,q_block_array,q_start_index,extracted_id_list):
    ...
    if len(g_init_dict[crush_id][rel_id+'_filtered_id_array']) >= ideal_lineup_members:
        ...
        create_lineup(relationship,acceptable_id_array)
        return
        ...
        if next_q_index >= len(q_block_array):
            ...
            process_batch_blocks(relationship,mf_index,q_block_array,next_q_index)
            return
#=================================================================    
...
def finished_process_mutual_friend(self,relationship,mf_index):
    ...
    if next_mf_index < len(g_init_dict[crush_id][rel_id+'_mutual_friend_array']):
        process_mutual_friend(relationship,next_mf_index)
    else:
        try_nonapi_cf_initialization(relationship)
...
def try_nonapi_cf_initialization(self,relationship):
    ...
    print "TRY METHOD 4"
    if len(g_init_dict[crush_id]['crush_friend_array']) < relationship['minimum_lineup_members']:
        initialize_fail(relationship,5)
        return
    else:
        ...
        batch_fetch_friends(relationship,0)
...
def batch_fetch_friends(self,relationship,cf_index):
    ...
    for x in range(cf_index,next_cf_index): # /iterate through next 18 friends
        ...
        thread.start_new_thread(start_single_friend_fetch,(relationship,x)) #initialize lineup asynchronously            
...
def start_single_friend_fetch(self,relationship,cf_index):
    ...
    if number_friends==None or number_friends < 5:
        ...
        finished_processing_friend(relationship,cf_index)
        return
    ...
    process_friend_block(relationship,cf_index,q_block_array,0)
...
def process_friend_block(self,relationship,cf_index,q_block_array,q_index):
    ...
    try:
        ...
        get_another_friend_block(relationship,cf_index,q_block_array,q_index)
        return
    ...
    if len(extracted_id_list)>0:
        filter_ids_by_admirer_conditions(relationship,cf_index,q_block_array,q_index,extracted_id_list)
    else:
...
def filter_ids_by_admirer_conditions(self,relationship,cf_index,q_block_array,q_index,extracted_id_list):
    ...
    if len(filtered_batch_id_array)>0:
        ...
        if len(g_init_dict[crush_id][rel_id+'_filtered_id_array']) >= relationship['ideal_lineup_members']:
            # we have enough lineup members so call final routine // if we get more ingore the others
            finalize_initialization(relationship)
            return
    finished_processing_friend(relationship,cf_index) #report in!
    return
...
def get_another_friend_block(self,relationship,cf_index,q_block_array,q_index):
    ...
    if q_index < len(q_block_array):
        process_friend_block(relationship,cf_index,q_block_array,q_index) 
    else:
        finished_processing_friend(relationship,cf_index)
    ...
    if g_init_dict[crush_id][rel_id+'_batch_friends_received'] == g_init_dict[crush_id][rel_id+'_batch_friends_requested']:
        if len(g_init_dict[crush_id][rel_id+'_filtered_id_array']) < ideal_lineup_members:
            ...
            if cf_index < len(g_init_dict[crush_id]['crush_friend_array']):
                batch_fetch_friends(relationship,cf_index)
                ...
                if len(g_init_dict[crush_id][rel_id+'_filtered_id_array']) >= minimum_lineup_members: 
                    ...
                    initialize_fail(relationship,2)
                    return
...
def finalize_initialization(self,relationship):
    ...
    create_lineup(relationship,g_init_dict[crush_id][rel_id+'_filtered_id_array'][:ideal_lineup_members])
...
def initialize_fail(self,relationship,status=2):
        ...
        cleanup_initialization_memory(relationship)
        return
