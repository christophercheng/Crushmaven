...
def send_mail_user_logged_in(user, header_string):
    if user.username not in ['100006341528806','1057460663','100004192844461','651900292','100003843122126','100007405598756']:
        send_mailgun_email('CrushMaven Admin <admin@crushmaven.com>','chris_h_cheng@hotmail.com',user.get_name() + ' logged in!','http://www.facebook.com/' + str(user.username) + " " + header_string,header_string)
        send_mailgun_email('CrushMaven Admin <admin@crushmaven.com>','6465732737@vmobl.com',user.get_name() + ' logged in!','http://www.facebook.com/'+str(user.username),"")
...
def send_mail_setup_target_response(full_name, short_name, first_name,pronoun_subject,pronoun_possessive,email_address):
    ...
    send_mailgun_email('CrushMaven <notifications@crushmaven.com>',email_address,short_name + ' responded to your setup!',html,text)
...
def send_mail_setup_recommendees_invited(email_address,full_name, short_name, first_name="",pronoun_subject="",pronoun_possessive="",send_time=None):    
    ...
    send_mailgun_email('CrushMaven <notifications@crushmaven.com>',email_address,short_name + ' notified your setup picks!',html,text,send_time)
