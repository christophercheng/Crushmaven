...
def write(request, recipients=None, form_classes=(WriteForm, AnonymousWriteForm), autocomplete_channels=None,
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            recipient_user=FacebookUser.objects.get(username=recipient_username)
            existing_messages = Message.objects.filter((Q(sender=user) & Q(recipient=recipient_user)) | (Q(sender=recipient_user) & Q(recipient=user))  ).order_by('parent')
            if len(existing_messages) == 0:
                raise Message.DoesNotExist
            parent=existing_messages[0]
    
            form = FullReplyForm(post_data, sender=user, recipient=recipient_user,
                channel=channel,
            ...
            if form.is_valid():
                ...
                print "parent thread id : " + str(parent.thread_id)
                print "success: " + str(success_url) + " next: " + str((next_url))
                return redirect('/messages/view/t/' + str(parent.thread_id) + '/')
     
        except Exception as e:
            print "EXEPTION: " + str(e)
            form = form_class(post_data, sender=user, channel=channel,
                user_filter=user_filter,
                exchange_filter=exchange_filter,
                max=max)
            if form.is_valid():
                ...
                if is_successful:
                    ...
                    messages.warning(request, _("Message rejected for at least one recipient."), fail_silently=True)
                last_message=Message.objects.filter(sender=user).latest('sent_at')
                return redirect('/messages/view/t/' + str(last_message.thread_id) + '/')
    else:
...
def reply(request, message_id, form_class=FullReplyForm, formatters=(format_subject,format_body), autocomplete_channel=None,
    ...
    if request.method == 'POST':
        ...
        if parent.recipient!=user:
            new_recipient = parent.recipient
        else:
            new_recipient = parent.sender
        form = form_class(post, sender=user, recipient=new_recipient or parent.email,
